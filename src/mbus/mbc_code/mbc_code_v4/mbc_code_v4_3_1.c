...
#define XO_DAY_START 5*60  // 5 am
#define XO_DAY_END  19*60   // 7 pm
...
#endif
const uint32_t LNT_INTERVAL[4] = {56.25, 112.5, 450, 1800};  // 56.25, 112.5, 450, 1800 seconds
...
const uint32_t PMU_TEMP_THRESH[6] = {0x190, 0x258, 0x3E8, 0x7D0, 0xC80, 0x1B58}; // {0, 10, 20, 30, 45, 60}	// FIXME: change these temporary values
...
volatile uint32_t radio_packet_count;
 ...
 #define LIGHT_MAX_REMAINDER 192-16*2-1*2-3*2-4*2-11-11 // 96-(CRC*2+TEMP_OR_LIGHT*2+PACKET_NUM*2+PACKET_HEADER*2+TIMESTAMP) = 122
...
uint16_t log2(uint64_t input) {
    ...
    mbus_write_message32(0xE9, out);
    for(i = 0; i < LOG2_RES; i++) {
        uint32_t new_result[4];
        long_int_mult(temp_result, LOG_CONST_ARR[i], new_result);
	// mbus_write_message32(0xE8, 0xEEE1);
        long_int_mult(input_storage, (1 << 15), input_storage);
	// mbus_write_message32(0xE8, 0xEEE2);

	int j;
	for(j = 0; j < LONG_INT_LEN; j++) {
	    mbus_write_message32(0xD6, new_result[j]);
	}
	for(j = 0; j < LONG_INT_LEN; j++) {
	    mbus_write_message32(0xD7, input_storage[j]);
	}
    ...
    // mbus_write_message32(0xEA, 0xEEEE);
...
static void light_left_shift_store(uint32_t data, uint8_t len) {
    ...
    light_code_storage[0] = light_code_storage[i] << len;
...
static void store_light() {
    if(light_storage_remainder < LIGHT_MAX_REMAINDER) {
        ...
        light_left_shift_store((xo_day_time_in_sec >> 6 & 0x7FF), 11); // final timestamp
        ...
        light_code_storage[3] |= light_code_storage[2] >> (32 - 24);
        ...
        mem_light_len += 3;
        mbus_copy_mem_from_local_to_remote_bulk(MEM_ADDR, (uint32_t*) (mem_light_addr + mem_light_len), light_code_storage, 2);
        mem_light_len += 3;
    }
}
...
uint8_t compress_light() {
    ...
    if(light_storage_remainder == LIGHT_MAX_REMAINDER) {
        ...
        if(cur_len_mode != 0xFF) {
            ...
            if(len_mode > cur_len_mode || reduce_len_counter >= 3 || lnt_l2_len >= ((1 << (6 - cur_len_mode)) - 1)) {
                store_l2_header();
        ...
        uint8_t needed_len = 2 + 3 + 2 + 6 - len_mode + len;
...
static void lnt_stop() {
    ...
    set_halt_until_mbus_tx();
    lnt_sys_light = (((*REG1 & 0xFFFFFF) << 24) | (*REG0)); // >> lnt_counter_base; // FIXME: this is wrong?
...
static uint16_t update_light_interval() {
    ...
    if(xo_day_time_in_sec <= XO_DAY_START) {
    
    }

    int lnt_meas_idx = 0;
    lnt_meas_time_mode = 0;
    return LNT_INTERVAL[lnt_meas_idx];
#endif
#ifndef LNT_MANUAL_PERIOD
    uint8_t i;
...
static void set_next_time(uint8_t idx, uint32_t step) {
    ...
    } while(!(diff > 5 && diff < step));    // give some margin of error and prevent overflow
}
...
static void operation_init( void ) {
    ...
    *REG_MBUS_WD = 0; // Disables Mbus watchdog timer
    ...
    mbus_write_message32(0xC4, log2(0x34600));
...
int main() {
    ...
    *NVIC_ISER = (1 << IRQ_WAKEUP | 1 << IRQ_GOCEP | 1 << IRQ_TIMER32 | 
    ...
    do {
		...
		mbus_write_message32(0xE6, 0xCCCC);
                pmu_setting_temp_based(0);
	    }

	    mbus_write_message32(0xEC, 0xCCCC);
