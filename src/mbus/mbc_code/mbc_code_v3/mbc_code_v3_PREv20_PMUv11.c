...
/*
 * Travel: 17-20 Jan, 2019
 * Fly in 16th, fly out 
 * Get flights tickets
 *
 * Pressure sensors data readout
 *
 * Talk to Gyouho and David about temp conversion chart
 * Limited space 
 * 7.5 kB of SRAM
 * top priority is integrate MRR
 * MEM layer get documentation; can put it to sleep
 * bring HOBO and PRESSURE sensors on the trip
 *
 * Use 0C setting for 9C and so on and so forth
 *
 * Only radio out when Vbat > 4V.
 * Calibrate our battery voltage for each unit
 * Check voltage in sleep
 * Check sleep current, shouldn't be too high
 ...
 *                 pmu
 *                 mem
 * Functions: temp measurement
 *            light meas
 *            periodic temp meas
 *            periodic light meas
 *            periodic temp + light read
 *            pmu settings
 *            voltage read
 */
