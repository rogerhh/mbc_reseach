...
//	disable_timerwd();
    ...
    delay(MBUS_DELAY);
    mbus_remote_register_write(FLP_ADDR, 0x07, 0x0);
    delay(MBUS_DELAY);
    delay(10000);

    uint32_t length = 29, cmd = 2;
    // set_halt_until_mbus_rx();
    ...
    while(!source);
    if(*REG1 != 0x3F) {
	    mbus_write_message32(0xE2, 0xDEADBE23);
    }
    // set_halt_until_mbus_tx();
    mbus_write_message32(0xBB, 0xDEADBEE2);
    ...
    while(!source);
    if(*REG1 != 0x2B) {
	    mbus_write_message32(0xE2, 0xDEADBE22);
    }

    uint32_t new_data[30];
    // set_halt_until_mbus_rx();
    mbus_copy_mem_from_remote_to_any_bulk(FLP_ADDR, (uint32_t*) 0x0, 0x1, new_data, 29);
    // set_halt_until_mbus_tx();
    delay(MBUS_DELAY);
    delay(10000);
    ...
    for(i = 0; i < 30; i++) {
    	mbus_write_message32(0xE2, new_data[i]);
	delay(MBUS_DELAY);
    }
