...
void update_system_time() {
    ...
    temp = (xo_sys_time - temp) >> XO_TO_SEC_SHIFT;
...
static void set_lnt_timer(uint32_t projected_end_time_in_sec) {
...
#define STATE_VERIFY 5
	    ...
	    if(++op_counter < 4) {
		projected_end_time = xo_sys_time_in_sec + 120;
	    }
	    else {
		goc_state = STATE_START;
	    	op_counter = 0;
		projected_end_time = start_hour_in_sec;
	    }
	    ...
	    goc_state = STATE_COLLECT;
            ...
            projected_end_time_in_sec = start_hour_in_sec + update_light_interval();
	}
		...
		if(radio_after_done) {
		    ...
		    pmu_set_sleep_low();
                ...
                if(store_temp_timestamp_in_sec >= xo_sys_time_in_sec) {
