 ...
 *
 *  v5.2:
 *      Using new sampling and compression algorithm
 *
 *  v5.2.1:
 *      Cleaned up v5.2 so that it compiles and looks cleaner
 *
 ******************************************************************************************/
...
volatile int8_t store_lnt_counter = 0;
...
volatile uint8_t mrr_send_enable = 1;
...
volatile uint32_t radio_packet_count;
 ...
 #define TEMP_MAX_REMAINDER 57 // 96-16-1-3-4-4-11 // 96-(CRC+TEMP_OR_LIGHT+PACKET_NUM+PACKET_HEADER+TIMESTAMP) = 57

volatile uint16_t log_light = 0, last_log_light = 0;
volatile uint8_t last_index = 0;

// 10-word unit
#define CACHE_MAX_SIZE 284 // 10 * 32 - 4 * 9   // the 9 bits are for one header
#define CACHE_LEN 10 // must be a multiple of 5
#define UNIT_HEADER_SIZE 31 // 17 + 7 + 7

// The cache is little endian
// cache[0] will be stored first

volatile uint32_t cache[CACHE_LEN];
volatile uint32_t cache_to_mem_addr = 0;
volatile uint16_t cache_remainder = CACHE_MAX_SIZE;

// TODO: fill in later
volatile uint16_t code[66];
volatile uint8_t code_len[66];

// unit header format
volatile int16_t end_idx_pos = -1;   // if end_idx_pos == -1, then we don't have a unit header
...
#define STORE_DATA_THRESHOLD 270 // = log2(0.22 * 1577) * 32
...
volatile uint16_t sum = 0xFFFF;
volatile uint32_t running_avg_time[8] = {0, 0, 0, 0, 0, 0, 0, 0};

volatile uint32_t start_day_time = 0;
volatile uint32_t cur_sunrise = 0, cur_sunset = 0, next_sunrise = 0, next_sunset = 0;
volatile uint32_t cur_edge = 0;
volatile uint32_t buffered_projected_time_in_sec = 0;

volatile bool is_morning = 0;
volatile uint8_t cur_index = 0;
volatile uint8_t interval_index = 0;
volatile uint8_t INTERVALS[8] = {32, 8, 2, 1, 1, 2, 8, 32};
volatile uint8_t SAMPLE_INDICES[8] = {12, 16, 32, 64, 96, 112, 116, 128};
volatile uint8_t day_count = 0;

volatile bool storing_data = false;
...
// FIXME: add a constant multiplier
...
// helper function for shifting cache
// if abs(len) >= 32, data should be 0
// used to shift cache-like arrays
// can be expanded to all arrays if needed
void right_shift_arr(uint32_t* arr, uint32_t data, int16_t len) {
    int8_t sign = len > 0? 1 : -1;
    int8_t start, end, back;
    ...
    back = end + sign; // for convenience. Maybe saves code space

    // clean data
    data &= (right_shift(1, -len) - 1);

    while(len > 32) {
        // shift word by word until len is less than 32
        int8_t i;
...
// Requires that len <= cache_remainder
// len can be up to 320 if data is 0
// otherwise len <= 32
// is_second_idx denotes if the new data is the second index of a header
// (in this case the data should be a dummy value 0)
// if true, then reset second_idx_pos
// if false, then update second_idx_pos
// the goal is to keep track of it so we can update it later
void store_data_to_cache(uint32_t data, 
                         int16_t len, 
                         uint8_t is_end_idx) {
    // process variables first before len is changed
    cache_remainder -= len;
    end_idx_pos = is_end_idx? 0 : (end_idx_pos + len);

    right_shift_arr(cache, data, -len);
}

// stores last_index as the end_idx of the current unit to the right position in the cache
void store_end_idx() {
    // create an array of the same size and shift the real idx in
    if(end_idx_pos != -1) {
        uint32_t dummy_cache[CACHE_LEN];
        uint8_t i;
        for(i = 0; i < CACHE_LEN; i++) {
            dummy_cache[i] = 0;
        }
        
        // use last_index because cur_index gets updated after storage
        dumm_cache[CACHE_LEN - 1] = last_index & 0x7F;
        right_shift_arr(dummy_cache, 0, end_idx_pos);

        for(i = 0; i < CACHE_LEN; i++) {
            // and the two caches to get the data in
            cache[i] |= dummy_cache;
        }
...
// requires that the cache has enough space 
// saving space for packet header but not adding it until radio send
// store cache data to mem and reset remainder count
void flush_unit() {
    if(cache_remainder < CACHE_MAX_SIZE) {
        ...
        mbus_copy_mem_from_local_to_remote_bulk(MEM_ADDR, (uint32_t*), cache_to_mem_addr, cache, CACHE_LEN - 1);
        cache_to_mem_addr += CACHE_LEN * 4; // CACHE_LEN * 10B
        cache_remainder = CACHE_MAX_SIZE;
    }
}

// stores the latest light measurement to cache
void store_light() {
    ...
    if(diff < 32 && diff >= -32) {
        code_idx = diff + 32;
        len_needed = code_len[code_idx];
    }
    else if(diff < 256 && diff >= -256) {
        code_idx = 64;
        len_needed = code_len[code_idx] + 9;
    }
    else {
        code_idx = 65;
        len_needed = code_len[65] + 11;
    }
    ...
    if(cache_remainder < len_needed) {
        flush_unit();
    }

    if(end_idx_pos == -1) {
        // edge timestamp 17 bits
        store_data_to_cache(cur_edge, 17, 0);
        // start idx
        store_data_to_cache(cur_index, 7, 0);
        // dummy end_idx
...
// using a function because multiple places use it
// Requires store_lnt_counter to only be > 0, 0 -1
// This is also where lnt_counter_base_is_set
void set_projected_end_time() {
    if(store_lnt_counter > 0) {
        // All 8-minute intervals should just go here
        projected_end_time_in_sec += XO_8_MIN;
        lnt_counter_base = 3;
    }
    else if(store_lnt_counter == 0) {
        projected_end_time_in_sec += (INTERVALS[interval_index] * 60);
        lnt_counter_base = INTERVALS[interval_index] - 1;
    }
    else {
        // in buffer state, if buffered_projected_time_in_sec is not within 10 minutes,
        // set 8 minute timer, else set to buffered_projected_time_in_sec
        // lnt_counter_base doesn't matter in these cases
        if(buffered_projected_time_in_sec - xo_sys_time_in_sec > XO_10_MIN) {
            projected_end_time_in_sec += XO_8_MIN;
        }
        else {
            projected_end_time_in_sec = buffered_projected_time_in_sec;
            store_lnt_counter = 1;
        }
    }
}

// stores last_index if there is a dangling unit header
// set the wakeup time for the next half-day. Wakeup time will be 8 minutes earlier
...
void start_half_day() {
    ...
    // set 1 hour margin for safety
    if(xo_day_time_in_sec <= NOON - XO_1_HR) {
        is_morning = true;
    }
    else if(xo_day_time_in_sec > XO_MAX_DAY_TIME_IN_SEC - XO_1_HR) {
        is_morning = true;
        new_day = true;
    }
    else {
    ...
    uint32_t margin1, margin2;

    if(is_morning) {
        storing_data = false; // morning starts with not storing data
        cur_sunrise = next_sunrise == 0? cur_sunrise : next_sunrise;
        next_sunrise = 0;
        cur_edge = cur_sunrise;

        margin1 = XO_8_MIN;
        margin2 = NOON - XO_32_MIN;
    }
    else {
        storing_data = true; // after starts with storing data
        cur_sunset = next_sunset == 0? cur_sunset : next_sunset;
        next_sunset = 0;
        cur_edge = cur_sunset;


        margin1 = NOON _ XO_8_MIN;
        margin2 = XO_MAX_DAY_TIME_IN_SEC - XO_32_MIN;
    }

    uint32_t next_day_time_in_sec = cur_edge - EDGE_MARGIN;
    cur_index = 0;

    // find first valid day time
    while(1) {
        if(next_day_time_in_sec >= margin1 && next_day_time_in_sec < margin2
                && ((next_day_time > xo_day_time_in_sec + XO_10_MIN) || new_day)) {
            break;
        }
        next_day_time_in_sec += (INTERVALS[interval_index] * 60);
    ...
    // set 8-minute buffer
    buffered_projected_time_in_sec = xo_sys_time_in_sec + next_day_time_in_sec
                                        - xo_day_time_in_sec - XO_8_MIN;

    if(next_day_time_in_sec <= xo_day_time_in_sec) {
        // wrap around case
        buffered_projected_time_in_sec += XO_MAX_DAY_TIME_IN_SEC;
    }
}

void operation_lnt_run() {
    if(sum == 0xFFFF) {
        // if uninitialized, initialize running avg
        uint8_t i;
        for(i = 0; i < 8; i++) {
            running_avg[i] = log_light;
            running_avg_time = xo_day_time_in_sec;
        }
        sum = log_light << 3;
    }
    else {
    ...
    uint32_t target = 0;
    if(is_morning && !next_sunrise && (sum >> 3) >= EDGE_THRESHOLD && day_count > 1) {
        target = 1; // sentinel to save code space
    }
    else if(!is_morning && !next_sunset && (sum >> 3) <= EDGE_THRESHOLD && day_count > 1) {
        target = 1; // sentinel
    }
    ...
    // determine next wakeup time in terms of day time
    // if at the end of this half-day, switch to the next half-day
    uint32_t next_lnt_day_time = projected_end_time_in_sec 
        + (INTERVALs[interval_index] * 60) 
        + xo_day_time_in_sec - xo_sys_time_in_sec;
    if(is_morning && ((next_lnt_day_time >= NOON - XO_10_MIN))) {
        start_half_day();
    }
    else if(!is_morning && ((next_lnt_day_time >= XO_MAX_DAY_TIME_IN_SEC - XO_10_MIN))) {
        start_half_day();
    }
    else if(cur_index >= SAMPLE_INDICES[7]) {
        start_half_day();
    }
    else {
        uint8_t temp = INTERVALS[interval_index];
        if(temp < 8) {
            store_lnt_counter = 0;
        }
        else {
            // this covers cases for 8 and 32
            store_lnt_counter = temp >> 3;
        }
    }
}
...
void radio_full_data() {
    ...
    for(i = 0; i < mem_light_len; i += 40) {
        // read data to cache
	set_halt_until_mbus_trx();
	mbus_copy_mem_from_remote_to_any_bulk(MEM_ADDR, (uint32_t*) (MEM_LIGHT_ADDR + i), PRE_ADDR, cache, CACHE_LEN - 1);
	set_halt_until_mbus_tx();

        uint8_t j;
        for(j = 0; j < 4; j++) {
            // store unit header
            // read the top 80 - unit header size and shift into radio data arr
        }
...
int main() {
    ...
    *NVIC_ISER = (1 << IRQ_WAKEUP | 1 << IRQ_GOCEP | 1 << IRQ_TIMER32 | 
    ...
    // reset lnt_counter_base everytime for safety
    lnt_counter_base = 0;
...
#define STATE_VERIFY 5
	    ...
	    radio_data_arr[0] = snt_sys_temp_code << 8 | (read_data_batadc & 0xFF);
	    radio_data_arr[1] = xo_sys_time_in_sec;
	    radio_data_arr[2] = lnt_meas_time;
	    mrr_send_radio_data(1);

            if(day_counter >= max_day_count && xo_day_time_in_sec >= end_day_time) {
		if(radio_after_done) {
		    ...
		    pmu_set_sleep_low();
                ...
                if(xo_sys_time_in_sec >= store_temp_timestamp_in_sec) {
                    store_temp_timestamp_in_sec += XO_32_MIN;    // increment by 30 minutes
                    compress_temp();
                }
