...
#include "/home/rogerhh/Dropbox/m3_monarch/pre_v20e/software/mbc_code/huffman_encodings_v5_2_2.h"
...
map<int, map<int, int>> temp_huffman_tree;
...
void read_light_huffman_tree() {
    ...
    for(int i = 0; i < 67; i++) {
        light_huffman_tree[light_code_lengths[i]].insert({light_diff_codes[i], i});
    }
    /*
...
void read_temp_huffman_tree() {
    for(int i = 1; i <= 5; i++) {
        temp_huffman_tree.insert({i, map<int, int>{}});
    }
    for(int i = 0; i < 5; i++) {
        temp_huffman_tree[temp_code_lengths[i]].insert({temp_diff_codes[i], i});
    }
}
...
struct LightUnit {
    ...
    int read_next() {
        ...
        while(1) {
            ...
            len++;
            if(len > 12) {
                cout << "can't find code for: " << temp << endl;
                return -1;
            }
            temp = (temp << 1) | b;
            // cout << "temp = " << temp << endl;
...
int main(int argc, char** argv) {
    if(argc != 2) {
        cerr << "Usage: ./decoder_v5_2_2.exe [CHIP_ID]" << endl;
        exit(1);
    }
    read_light_huffman_tree();
    read_temp_huffman_tree();
    chip_id = atoi(argv[1]);
    string packet;
    LightUnit lu;
    TempUnit tu;
